...
import { CreditCard, AlertTriangle, CheckCircle, Calendar, User } from "lucide-react";
import { format, isBefore } from "date-fns";

interface PayableBreakdownProps {
  onClose: () => void;
}

export function PayableBreakdown({ onClose }: PayableBreakdownProps) {
  ...
  const payableBreakdown = useMemo(() => {
    ...
    // Get all taken loans (money you owe others)
    ...
    const completedTakenLoans = takenLoans.filter(loan => loan.status === 'completed');
    
    const totalPayable = activeTakenLoans.reduce((sum, loan) => sum + loan.remainingAmount, 0);
    const overdueAmount = overdueTakenLoans.reduce((sum, loan) => sum + loan.remainingAmount, 0);
    const totalPaid = activeTakenLoans.reduce((sum, loan) => sum + loan.totalPaid, 0);
    const totalCompleted = completedTakenLoans.reduce((sum, loan) => sum + loan.amount, 0);
    
    // Group by account
    const byAccount = activeTakenLoans.reduce((acc, loan) => {
    ...
    return {
      ...
      totalPaid,
      totalCompleted,
      activeCount: activeTakenLoans.length,
      overdueCount: overdueTakenLoans.length,
      completedCount: completedTakenLoans.length,
      byAccount,
      activeLoans: activeTakenLoans,
      overdueLoans: overdueTakenLoans,
    };
  }, [loans, accounts]);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-2">
          Money you owe others from taken loans
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrency(payableBreakdown.totalPayable)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrency(payableBreakdown.overdueAmount)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {formatCurrency(payableBreakdown.totalPaid)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-600">
              {formatCurrency(payableBreakdown.totalCompleted)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {/* By Account */}
      {Object.keys(payableBreakdown.byAccount).length > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(payableBreakdown.byAccount).map(([accountName, data]) => (
      ...
      {/* Active Loans List */}
      {payableBreakdown.activeLoans.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <CreditCard className="h-5 w-5" />
              Active Loans ({payableBreakdown.activeCount})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {payableBreakdown.activeLoans.map((loan) => {
      ...
      {/* Overdue Loans Alert */}
      {payableBreakdown.overdueLoans.length > 0 && (
        <Card className="border-red-200 bg-red-50 dark:bg-red-950/20">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2 text-red-600">
              <AlertTriangle className="h-5 w-5" />
              Overdue Loans ({payableBreakdown.overdueCount})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {payableBreakdown.overdueLoans.map((loan) => (
      ...
      {payableBreakdown.activeLoans.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <h3 className="text-lg font-medium mb-2">No Active Payables</h3>
            <p className="text-muted-foreground">
              You don't have any active loans taken at the moment.
            </p>
          </CardContent>
        </Card>
      )}
